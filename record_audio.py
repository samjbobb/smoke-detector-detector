...
def build_ffmpeg_command(input_format, device_input, output_file, duration=None):
    """Build ffmpeg command with proper option ordering."""
    # Base command with global options
    cmd = ['ffmpeg', '-y']
    
    # Input options
    cmd.extend(['-thread_queue_size', '8192'])  # Large buffer for all platforms
    
    # Input specification
    cmd.extend(['-f', input_format, '-i', device_input])
    
    # Output options - simplified to avoid conflicts
    cmd.extend([
...
def test_audio_device(device_name, input_format):
    ...
    cmd = build_ffmpeg_command(input_format, device_input, '/tmp/audio_test.wav', duration=1)
...
def record_audio(device_name, input_format):
    ...
    device_input = f":{device_name}" if platform.system().lower() == 'darwin' else device_name
    cmd = build_ffmpeg_command(input_format, device_input, output_file)
